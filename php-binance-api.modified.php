...
class API {
	...
	public function marketBuy($symbol, $quantity) {
		return $this->order("BUY", $symbol, $quantity, null, "MARKET", $flags = []);
	}
	public function marketSell($symbol, $quantity) {
		return $this->order("SELL", $symbol, $quantity, null, "MARKET", $flags = []);
	}
	...
	public function priceTicker($symbol = null) {
		return $this->request("v3/ticker/price", compact('symbol'));
	}
	public function bookTicker($symbol = null) {
		return $this->request("v3/ticker/bookTicker", compact('symbol'));
	}
	...
	private function request($url, $params = [], $method = "GET") {
		...
		return json_decode(file_get_contents($this->base.$url.'?'.$query, false, $context));
	}
	...
	private function apiRequest($url, $method = "GET") {
		...
		$context = stream_context_create($opt);
		return json_decode(file_get_contents($this->base.$url, false, $context));
	}
	...
	public function candlesticks($symbol, $interval = "5m", $startTime = null, $endTime = null) {
		if ( !isset($this->charts[$symbol]) ) $this->charts[$symbol] = [];
		$params = compact('symbol', 'interval', 'startTime', 'endTime');
